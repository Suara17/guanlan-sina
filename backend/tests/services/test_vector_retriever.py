...
class StubChromaVectorStoreService:
    def __init__(self, records: list[dict] | None = None) -> None:
        ...
        self.has_data = bool(self.records)
        self.last_retriever_kwargs: dict[str, object] | None = None

    def similarity_search(self, query_text: str, *, top_k: int, filters=None):
        _ = query_text, top_k, filters
        return self.records

    def get_retriever(self, *, top_k: int, filters=None):
        self.last_retriever_kwargs = {"top_k": top_k, "filters": filters}
        return "stub-langchain-retriever"
...
def test_vector_retriever_prefers_chroma_results(tmp_path: Path, monkeypatch):
    chunks_path = tmp_path / "chunks.jsonl"
    chunks_path.write_text("", encoding="utf-8")
    chroma_records = [
        {
            "document_id": "spi-manual",
            "chunk_id": "spi-manual:p18:c02",
            "source_file": "docs/知识图谱/downloads/spi-manual.pdf",
            "title": "SE 300 Solder Paste Inspection User Guide",
            "page": 18,
            "section": "Setup Check",
            "text": "Setup check requires stencil alignment verification and conveyor calibration.",
            "text_preview": "Setup check requires stencil alignment verification and conveyor calibration.",
            "keywords": ["setup check", "stencil alignment"],
            "line_types": ["SMT"],
    ...
    retriever = VectorRetriever(
        DocumentIndexService(chunks_path=chunks_path),
        chroma_vector_store_service=StubChromaVectorStoreService(chroma_records),
    )

    result = retriever.retrieve(
        QARequest(question="SPI设备手册里的 setup check 怎么做", line_type="SMT", top_k=3)
    )

    assert len(result.hits) == 1
    assert result.hits[0]["document_id"] == "spi-manual"
    assert result.hits[0]["similarity_score"] > 0.9


def test_vector_retriever_builds_langchain_retriever_from_chroma(monkeypatch):
    chroma_service = StubChromaVectorStoreService(
        [
            {
                "document_id": "spi-manual",
                "chunk_id": "spi-manual:p18:c02",
    ...
    monkeypatch.setattr(
        "app.services.retrievers.vector_retriever.settings.CHROMA_TOP_K",
        6,
    )

    retriever = VectorRetriever(
        DocumentIndexService(chunks_path=Path("__missing__.jsonl")),
        chroma_vector_store_service=chroma_service,
    )

    result = retriever.get_langchain_retriever(
        QARequest(question="SPI设备手册里的 setup check 怎么做", line_type="SMT", top_k=3)
    )

    assert result == "stub-langchain-retriever"
    assert chroma_service.last_retriever_kwargs == {
        "top_k": 6,
        "filters": {"line_type_primary": "SMT"},
    }
