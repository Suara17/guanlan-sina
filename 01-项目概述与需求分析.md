...
- **修订说明**: 适配全栈模板（FastAPI + Vue.js），替换原NestJS + React技术栈；调整消息中间件为Celery + Redis；扩展数据库支持；保持核心功能需求不变。
...
<template>
  <knowledge-graph
    ...
    :highlight-path="evidencePath"
  />
</template>

<script setup>
import KnowledgeGraph from '@company/viz-components/knowledge-graph.vue';
</script>
```
...
| v1.1 | 2026-01-12 | 架构团队 | 适配FastAPI + Vue.js模板，替换技术栈；调整代码示例、结构和部署配置 |