...
type SinanQaSource = 'global' | 'dashboard' | 'sinan' | 'gewu' | 'kernel' | 'huntian' | 'tianchou' | 'zhixing'
...
const shouldShowFloatingSinan = (pathname: string): boolean => {
  ...
  return (
    normalizedPath === '/app/' ||
    normalizedPath === '/app/sinan' ||
    normalizedPath === '/app/gewu'
  )
}
  ...
  return (
    <>
      {!state.open && (
        <button
          type="button"
          onClick={() => onOpen(state.source, state.context, state.draftQuestion || defaultQuestion)}
...
const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [sinanQaState, setSinanQaState] = useState<SinanQaState>({
    ...
    context: getSinanQaContext(location.pathname, location.search, location.state),
    draftQuestion: '',
  })
