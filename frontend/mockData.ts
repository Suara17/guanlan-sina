...
// 合并所有知识图谱数据（用于无特定异常ID时的全局展示）
export const getAllKnowledgeGraphsMerged = (): KnowledgeGraph => {
  const allGraphs = [
    KNOWLEDGE_GRAPH_SMT_001,
    KNOWLEDGE_GRAPH_SMT_002,
    KNOWLEDGE_GRAPH_PCB_003,
    KNOWLEDGE_GRAPH_SMT_004,
    KNOWLEDGE_GRAPH_SMT_005,
    KNOWLEDGE_GRAPH_PCB_006,
    KNOWLEDGE_GRAPH_PCB_007,
    KNOWLEDGE_GRAPH_PCB_008,
    KNOWLEDGE_GRAPH_3C_009,
    KNOWLEDGE_GRAPH_PCB_010,
    KNOWLEDGE_GRAPH_SMT_011,
  ]
  ...
  allGraphs.forEach((graph) => {
    ...
    graph.nodes.forEach((node) => {
      const newNode: KnowledgeNode = {
        ...node,
        id: `${graph.anomalyId}-${node.id}`,
        label: `${graph.anomalyId.toUpperCase()}: ${node.label}`,
      }
    ...
    graph.edges.forEach((edge) => {
      const newEdge: KnowledgeEdge = {
        ...edge,
        id: `${graph.anomalyId}-${edge.id}`,
        source: `${graph.anomalyId}-${edge.source}`,
        target: `${graph.anomalyId}-${edge.target}`,
      }
...
export const SOLUTIONS_WITH_COST: Record<string, SolutionWithCost[]> = {
  'smt-001': [
    {
      id: 'sol-smt-001-a',
      title: '方案A：立即停机更换吸嘴',
      ...
      risk: 'low',
      repairCost: 2500,
      deliveryImpactCost: 8000,
      qualityRiskCost: 500,
      downtimeCost: 12000,
      totalExpectedLoss: 23000,
      implementationTimeHours: 0.25,
      successRate: 0.92,
      riskLevel: 'low',
      costMatrix: {
        repairCost: 2500,
    ...
    },
    {
      ...
      risk: 'medium',
      repairCost: 0,
      deliveryImpactCost: 1000,
      qualityRiskCost: 3000,
      downtimeCost: 0,
      totalExpectedLoss: 4000,
      implementationTimeHours: 0.03,
      successRate: 0.75,
      riskLevel: 'medium',
      costMatrix: {
        repairCost: 0,
  ...
  'smt-002': [
    {
      ...
      risk: 'low',
      repairCost: 500,
      deliveryImpactCost: 6000,
      qualityRiskCost: 800,
      downtimeCost: 8000,
      totalExpectedLoss: 15300,
      implementationTimeHours: 0.5,
      successRate: 0.88,
      riskLevel: 'low',
      costMatrix: {
        repairCost: 500,
    ...
    },
    {
      ...
      risk: 'medium',
      repairCost: 0,
      deliveryImpactCost: 24000,
      qualityRiskCost: 200,
      downtimeCost: 0,
      totalExpectedLoss: 24200,
      implementationTimeHours: 0.1,
      successRate: 0.95,
      riskLevel: 'medium',
      costMatrix: {
        repairCost: 0,
  ...
  'pcb-003': [
    {
      ...
      risk: 'low',
      repairCost: 3000,
      deliveryImpactCost: 12000,
      qualityRiskCost: 1000,
      downtimeCost: 15000,
      totalExpectedLoss: 31000,
      implementationTimeHours: 1.0,
      successRate: 0.85,
      riskLevel: 'low',
      costMatrix: {
        repairCost: 3000,
    ...
    },
    {
      ...
      risk: 'low',
      repairCost: 15000,
      deliveryImpactCost: 48000,
      qualityRiskCost: 500,
      downtimeCost: 60000,
      totalExpectedLoss: 123500,
      implementationTimeHours: 24.0,
      successRate: 0.98,
      riskLevel: 'low',
      costMatrix: {
        repairCost: 15000,
  ...
  '3c-009': [
    {
      ...
      risk: 'low',
      repairCost: 800,
      deliveryImpactCost: 4000,
      qualityRiskCost: 200,
      downtimeCost: 6000,
      totalExpectedLoss: 11000,
      implementationTimeHours: 0.5,
      successRate: 0.92,
      riskLevel: 'low',
      costMatrix: {
    ...
    },
    {
      ...
      repairCost: 200,
      deliveryImpactCost: 8000,
      qualityRiskCost: 1500,
      downtimeCost: 12000,
      totalExpectedLoss: 21700,
      implementationTimeHours: 1.0,
      successRate: 0.8,
      riskLevel: 'medium',
      costMatrix: {
...
export const getSolutionsByAnomalyId = (anomalyId: string): SolutionWithCost[] => {
  return SOLUTIONS_WITH_COST[anomalyId] || []
}

export const ANOMALY_ANALYSIS_DATA: Record<string, AnomalyAnalysis> = {
  'smt-001': {
    id: 'smt-001',
    lineId: 'smt-a01',
    stationId: 'station-5',
    defectType: '贴装精度下降',
    severity: 'error',
    detectedAt: '2024-02-23T17:13:00Z',
    status: 'open',
    rootCause: '产线布局密度过大（间距<0.8m）形成热岛效应，阻碍自然对流散热',
    ...
    causationChain: [
      ...
      { step: 3, description: '产线布局密度过大（间距<0.8m）形成热岛效应', type: 'root_cause' },
    ],
    solutions: [],
  },
  'smt-002': {
    ...
    defectType: '后续贴装不良',
    severity: 'warning',
    detectedAt: '2024-02-23T11:40:00Z',
    status: 'open',
    rootCause: '冷却区有效长度（2米）与传送带速度不匹配，无法提供足够的热交换时间',
    rootCauseConfidence: 0.88,
    causationChain: [
      { step: 1, description: '后续贴装不良', type: 'phenomenon' },
      { step: 2, description: 'PCB出炉温度超过50℃，在机械传输中发生位移', type: 'direct_cause' },
      { step: 3, description: '冷却区有效长度与传送带速度不匹配', type: 'root_cause' },
    ],
    solutions: [],
  },
  'pcb-003': {
    ...
    defectType: '曝光精度下降',
    severity: 'error',
    detectedAt: '2024-02-23T11:54:00Z',
    status: 'open',
    rootCause: '传输距离过长（8米）且无环境温度控制，导致板面温度在进入曝光机前波动过大',
    rootCauseConfidence: 0.85,
    causationChain: [
      { step: 1, description: '曝光精度下降', type: 'phenomenon' },
      { step: 2, description: 'PCB板在传输过程中发生非均匀热收缩', type: 'direct_cause' },
      { step: 3, description: '传输距离过长且无环境温度控制', type: 'root_cause' },
    ],
    solutions: [],
  },
  '3c-009': {
    ...
    defectType: '外壳开裂',
    severity: 'critical',
    detectedAt: '2024-02-23T11:30:00Z',
    status: 'open',
    rootCause: '电批内部离合器磨损，导致扭力设定值失效（打滑或锁死）',
    rootCauseConfidence: 0.9,
    causationChain: [
      { step: 1, description: '外壳开裂', type: 'phenomenon' },
      { step: 2, description: '电批输出扭力超过外壳材料屈服极限', type: 'direct_cause' },
...
export const getAnomalyAnalysis = (anomalyId: string): AnomalyAnalysis | null => {
  return ANOMALY_ANALYSIS_DATA[anomalyId] || null
}
