...
### 数据源配置
- **主数据源**: Neo4j图数据库（生产环境）
- **备用数据源**: 模拟数据（开发/测试环境或Neo4j不可用时）
- **自动降级**: 当Neo4j不可用时，系统自动切换到模拟数据，确保功能可用性
- **数据适配器**: `KnowledgeGraphAdapter` 负责处理不同数据源间的切换逻辑

### API集成
- 通过后端API与Neo4j数据库交互
- 前端不直接连接数据库，所有请求通过后端API代理
- 支持缓存机制，提高数据加载速度
