...
import { Workshop } from '../factoryData';
import { ProductionLine3D } from './ProductionLine3D';
import { Text, Html, Box, Instance, Instances } from '@react-three/drei';
...
const StorageRack = ({ position, rotation = [0, 0, 0] }: { position: [number, number, number], rotation?: [number, number, number] }) => (
  <group position={position} rotation={rotation as any}>
    {/* Frame */}
    <Box args={[0.1, 3, 0.1]} position={[-1.5, 1.5, -0.5]}><meshStandardMaterial color="#1e3a8a" /></Box>
    <Box args={[0.1, 3, 0.1]} position={[1.5, 1.5, -0.5]}><meshStandardMaterial color="#1e3a8a" /></Box>
    <Box args={[0.1, 3, 0.1]} position={[-1.5, 1.5, 0.5]}><meshStandardMaterial color="#1e3a8a" /></Box>
    <Box args={[0.1, 3, 0.1]} position={[1.5, 1.5, 0.5]}><meshStandardMaterial color="#1e3a8a" /></Box>
    ...
    {/* Boxes on shelves */}
    <Box args={[0.8, 0.6, 0.8]} position={[-0.8, 0.8, 0]}><meshStandardMaterial color="#d97706" /></Box>
    <Box args={[0.8, 0.6, 0.8]} position={[0.2, 0.8, 0]}><meshStandardMaterial color="#b45309" /></Box>
    <Box args={[0.8, 0.5, 0.8]} position={[0.5, 1.8, 0.2]}><meshStandardMaterial color="#d97706" /></Box>
  </group>
);
...
const SafetyZone = ({ width, depth, position }: { width: number, depth: number, position: [number, number, number] }) => (
  <group position={position}>
    {/* Yellow Outline */}
...
export const Workshop3D: React.FC<Workshop3DProps> = ({ workshop, position = [0, 0, 0], onClick }) => {
         ...
         <Html position={[10, 5, 6]} center distanceFactor={25} style={{ pointerEvents: 'none' }}>
            <div className="bg-slate-900/90 text-white px-4 py-2 rounded-lg shadow-xl border border-slate-700 backdrop-blur-sm cursor-pointer hover:scale-105 transition-transform min-w-[150px]">
              ...
              <div className="flex items-center gap-2 mt-1 justify-between">
                 <span className="text-xs text-slate-400 font-mono uppercase">Status</span>
                 <div className="flex items-center gap-1">
                    <div className={`w-2 h-2 rounded-full ${workshop.status === 'running' ? 'bg-green-500' : 'bg-red-500'}`}></div>
                    <span className={`text-xs font-bold ${workshop.status === 'running' ? 'text-green-400' : 'text-red-400'}`}>{workshop.status.toUpperCase()}</span>
                 </div>
              </div>
            </div>
         </Html>
