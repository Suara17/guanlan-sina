...
import { Station, StatusType } from '../factoryData';
...
const SignalTower = ({ status, position }: { status: StatusType; position: [number, number, number] }) => {
  const colors = {
  ...
  return (
    <group position={position}>
      ...
      <Cylinder args={[0.06, 0.06, 0.15]} position={[0, 0.5, 0]}>
        <meshStandardMaterial color={colors.green} emissive={colors.green} emissiveIntensity={status === 'running' ? 2 : 0} />
      </Cylinder>
      {/* Yellow Light */}
      <Cylinder args={[0.06, 0.06, 0.15]} position={[0, 0.35, 0]}>
        <meshStandardMaterial color={colors.yellow} emissive={colors.yellow} emissiveIntensity={status === 'idle' ? 2 : 0} />
      </Cylinder>
      {/* Red Light */}
      <Cylinder args={[0.06, 0.06, 0.15]} position={[0, 0.2, 0]}>
        <meshStandardMaterial color={colors.red} emissive={colors.red} emissiveIntensity={status === 'error' ? 2 : 0} />
      </Cylinder>
    </group>
  );
};
...
const MonitorArm = ({ position, rotation = [0, 0, 0] }: { position: [number, number, number], rotation?: [number, number, number] }) => (
...
// Safety Glass Door
const SafetyDoor = ({ width, height, position }: { width: number, height: number, position: [number, number, number] }) => (
  <group position={position}>
    {/* Frame */}
        ...
        return (
          <group>
            ...
            <group position={[0, 0.8, 0.75]}>
               {[...Array(10)].map((_, i) => (
                 <Box key={i} args={[0.15, 0.4, 0.3]} position={[(i - 4.5) * 0.2, 0, 0]} rotation={[0.4, 0, 0]}>
        ...
        <Html position={[0, 2.5, 0]} center distanceFactor={10} style={{ pointerEvents: 'none' }}>
          <div className="bg-slate-900/90 text-white p-3 rounded-lg shadow-xl border border-slate-700 backdrop-blur-sm text-xs min-w-[150px]">
            ...
            <div className="grid grid-cols-2 gap-x-2 gap-y-1">
              ...
              <span style={{ color: STATUS_COLORS[station.status] }} className="font-mono uppercase">{station.status}</span>
