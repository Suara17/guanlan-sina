/* biome-ignore-all lint/suspicious/noArrayIndexKey: 程序化装饰网格使用固定生成顺序，索引键不会影响业务状态。 */
import { Box, Html, Instance, Instances, Text } from '@react-three/drei'
import { useFrame } from '@react-three/fiber'
import React, { useMemo, useRef, useState } from 'react'
import * as THREE from 'three'
import type { Factory } from '../factoryData'
...
const RoofEquipment = ({
  ...
  const items = useMemo(() => {
    return [...Array(count)].map((_, _i) => ({
      pos: [(Math.random() - 0.5) * areaSize[0], 0, (Math.random() - 0.5) * areaSize[1]] as [
        number,
        number,
        number,
      ],
      size: [0.5 + Math.random() * 1.5, 0.5 + Math.random() * 0.8, 0.5 + Math.random() * 1.5] as [
        number,
        number,
        number,
      ],
    ...
    <group>
      {items.map((item, i) => (
        <group key={i} position={item.pos}>
          ...
          {item.type === 'ac' && (
            <Box
              args={[item.size[0] * 0.8, 0.1, item.size[2] * 0.8]}
              position={[0, item.size[1], 0]}
            >
...
const ModernFactoryHall = ({
  ...
  return (
    <group position={position}>
      ...
      <Box
        args={[width - 0.2, height - 1.0, depth - 0.2]}
        position={[0, 1.0 + (height - 1.0) / 2, 0]}
        castShadow
        receiveShadow
      >
      ...
      <group position={[0, height, 0]}>
        ...
        <Box
          args={[wallThickness, 0.8, depth - 1]}
          position={[width / 2 - wallThickness / 2, 0.4, 0]}
        >
          <meshStandardMaterial color="#f1f5f9" />
        </Box>
        <Box
...
export const FactoryBuilding3D: React.FC<FactoryBuildingProps> = ({ factory }) => {
        ...
        <Html position={[0, 12, 10]} center distanceFactor={25} style={{ pointerEvents: 'none' }}>
          <div className="bg-slate-900/95 backdrop-blur px-5 py-3 rounded-xl shadow-2xl border border-slate-700 text-white transform transition-all min-w-[200px]">
            ...
            <div className="flex justify-between items-center">
              <span className="text-slate-400 text-xs uppercase tracking-wider">状态</span>
              <span
            ...
            <div className="mt-2 text-xs text-slate-500 text-center">点击进入厂区</div>
          </div>
        </Html>
      )}
    </group>
